...
ESTATUS_OPCIONES = ["DISPERSADO", "EN ONBOARDING", "PENDIENTE CLIENTE", "PROPUESTA", "PENDIENTE DOC", "REC SOBREENDEUDAMIENTO", "REC NO CUMPLE POLITICAS", "REC EDAD"]
SEGUNDO_ESTATUS_OPCIONES = ["", "DISPERSADO", "EN ONBOARDING", "PEND.ACEPT.CLIENTE", "APROB.CON PROPUESTA", "PEND.DOC.PARA EVALUACION", "RECH.SOBREENDEUDAMIENTO", "RECH. TIPO PENSION", "RECH.EDAD"]
...
COLUMNS = [
    ...
    "score","telefono","correo","analista"
]
...
def guardar_clientes(df: pd.DataFrame):
    """
    Guarda el DataFrame de clientes en CLIENTES_CSV asegurando las columnas esperadas
    y evitando NaNs; muestra un error en UI si falla la escritura.
    """
    try:
        if df is None:
            return
        # Asegurar existencia de columnas esperadas y ordenarlas
        for c in COLUMNS:
            if c not in df.columns:
                df[c] = ""
        cols = [c for c in COLUMNS if c in df.columns]
        df_to_save = df[cols].copy().fillna("").astype(str)
        # Escribir usando pandas (soporta Path)
...
# Asegurar recarga limpia y recomputar índices/valores derivados en cada rerun
try:
    df_cli = cargar_clientes()
except Exception:
    df_cli = df_cli
...
st.sidebar.button("🔁 Restablecer filtros", key="btn_reset_filters", on_click=_reset_filters)
...
df_ver = df_cli[suc_mask & est_mask & asesor_mask].copy()
...
with tab_cli:
    ...
    if df_ver.empty:
        ...
        if ids_quick:
            ...
            with col_q1:
                cid_quick = st.selectbox("Cliente", ids_quick, format_func=lambda x: f"{x} - {df_ver.loc[df_ver['id']==x, 'nombre'].values[0]}")
                nombre_q = df_ver.loc[df_ver["id"]==cid_quick, "nombre"].values[0]
                estatus_actual = df_cli.loc[df_cli["id"]==cid_quick, "estatus"].values[0]
                seg_actual = df_cli.loc[df_cli["id"]==cid_quick, "segundo_estatus"].values[0]
            with col_q2:
        ...
        with col_del:
            ...
            if can("delete_client"):
                    if ids_quick:
                        ...
                        opts = [""] + [f"{cid} - {df_cli.loc[df_cli['id']==cid, 'nombre'].values[0]}" if ('nombre' in df_cli.columns and not df_cli.loc[df_cli['id']==cid, 'nombre'].empty) else str(cid) for cid in ids_quick]
                    ...
                    if cid_del and st.button("🗑️ Eliminar seleccionado"):
                        ...
                        try:
                            nombre_del = df_cli.loc[df_cli["id"]==cid_del, "nombre"].values[0] if "nombre" in df_cli.columns else ""
                        except Exception:
                            nombre_del = ""
                        actor = (current_user() or {}).get("user") or (current_user() or {}).get("email")
...
with tab_asesores:
    ...
    # Trabajamos con el dataframe filtrado por el sidebar (df_ver)
    if df_ver.empty:
        st.info("No hay clientes con los filtros seleccionados.")
    else:
        # Normalizar vacío para agrupar
        df_agrupe = df_ver.copy()
        df_agrupe["asesor"] = df_agrupe["asesor"].replace({"": "(Sin asesor)"})

        # --- Filtro por rango de fechas (opcional) ---
        # Elegir columna de fecha preferida
        date_col = None
        for c in ("fecha_ingreso", "fecha_dispersion"):
            if c in df_agrupe.columns:
                date_col = c
                break

        if date_col is not None:
            try:
                df_agrupe[date_col] = pd.to_datetime(df_agrupe[date_col], errors="coerce")
                min_date = df_agrupe[date_col].min()
                max_date = df_agrupe[date_col].max()
                if not pd.isna(min_date) and not pd.isna(max_date):
                    ...
                    try:
                        ...
                        df_agrupe = df_agrupe.loc[mask_date].copy()
                    except Exception:
                        # si algo falla con la comparación, no filtrar
                        pass
                else:
                    st.info("No hay valores de fecha válidos para filtrar en la base de asesores.")
            except Exception:
                # no bloquear la vista si la conversión falla
                pass

        # ---- Resumen por asesor: conteo total y por estatus ----
        conteo_por_asesor = (
            df_agrupe.groupby("asesor", dropna=False)
            .size()
        ...
        with col_l:
            ...
            # ordenar detalle por fechas antes de mostrar
            df_detalle_for_view = df_agrupe[COLUMNS].copy()
            ...
            xls_sel = build_excel_bytes({"Filtrados": sort_df_by_dates(df_agrupe[COLUMNS])})
            ...
            for a in conteo_por_asesor["asesor"].tolist():
                dfx = df_agrupe[df_agrupe["asesor"] == a][COLUMNS].copy()
                hojas[a] = sort_df_by_dates(dfx)
            if hojas:
        ...
        with col_r:
            ...
            try:
                # Preparar datos y dimensiones
                df_plot = conteo_por_asesor.copy()
                df_plot["asesor"] = df_plot["asesor"].astype(str)
                maxv = int(df_plot["clientes"].max() if not df_plot.empty else 0)
                n_ases = len(df_plot)
                height_px = max(240, n_ases * 42)

                # Gráfica horizontal simple y robusta:
                bars = (
                    alt.Chart(df_plot)
                    ...
                    .encode(
                        ...
                        y=alt.Y(
                            "asesor:N",
                            sort=alt.EncodingSortField(field="clientes", order="descending"),
                            title="Asesor"
                        ),
                        tooltip=[alt.Tooltip("asesor:N"), alt.Tooltip("clientes:Q", format="d")]
                    )
                )
                ...
                labels = (
                    ...
                    .encode(
                        x=alt.X("clientes:Q"),
                        y=alt.Y("asesor:N", sort=alt.EncodingSortField(field="clientes", order="descending")),
                        text=alt.Text("clientes:Q", format="d")
                    )
                )
        ...
        piv = (
            df_agrupe
            .pivot_table(index="asesor", columns="estatus", values="id", aggfunc="count", fill_value=0)
            .reset_index()
        )
...
with tab_docs:
    ...
    if df_cli.empty:
        ...
        ids = df_cli["id"].tolist()
        cid_sel = st.selectbox(
            "Selecciona cliente",
            [""] + ids,
            format_func=lambda x: "—" if x == "" else f"{x} - {df_cli.loc[df_cli['id']==x, 'nombre'].values[0]}",
            key="docs_cid_sel"
        )
        if cid_sel:
            estatus_cliente_sel = df_cli.loc[df_cli["id"] == cid_sel, "estatus"].values[0]
            ...
            with st.form(form_key, clear_on_submit=True):
                ...
                if submitted:
                    ...
                    if subidos_lote:
                        ...
                        actor = (current_user() or {}).get("user") or (current_user() or {}).get("email")
                        try:
                            nombre_x = df_cli.loc[df_cli["id"]==cid_sel, "nombre"].values[0]
                            est_x    = df_cli.loc[df_cli["id"]==cid_sel, "estatus"].values[0]
                            seg_x    = df_cli.loc[df_cli["id"]==cid_sel, "segundo_estatus"].values[0]
                        except Exception:
            ...
            if files:
                                    ...
                                    with a1:
                                        ...
                                        if st.button("Eliminar", key=del_key):
                                            try:
                                                ...
                                                try:
                                                    actor = (current_user() or {}).get("user") or (current_user() or {}).get("email")
                                                    append_historial(str(cid_sel), df_cli.loc[df_cli['id']==cid_sel, 'nombre'].values[0], "", "", "", "", f"Eliminado: {f.name}", action="DOCUMENTOS", actor=actor)
                                                except Exception:
                                                    pass
                ...
                if st.button("📦 Descargar carpeta (ZIP)", key=f"zip_cliente_{cid_sel}"):
                    ...
                    try:
                        actor = (current_user() or {}).get("user") or (current_user() or {}).get("email")
                        append_historial(str(cid_sel), df_cli.loc[df_cli['id']==cid_sel, 'nombre'].values[0], "", "", "", "", f"ZIP cliente preparado ({len(files)} archivos)", action="DESCARGA ZIP CLIENTE", actor=actor)
                    except Exception:
                        pass
                ...
                if st.session_state.get(f"_last_zip_{cid_sel}"):
                    st.download_button(
                        ...
                        file_name=f"{safe_name(cid_sel)}_{safe_name(df_cli.loc[df_cli['id']==cid_sel, 'nombre'].values[0])}.zip",
            ...
            if can("delete_client"):
                ...
                if st.button(f"🗑️ Eliminar {cid_sel}", key=f"del_{cid_sel}"):
                    ...
                    nombre_del = df_cli.loc[df_cli["id"]==cid_sel, "nombre"].values[0] if "nombre" in df_cli.columns else ""
                    actor = (current_user() or {}).get("user") or (current_user() or {}).get("email")
...
with tab_import:
    ...
    import_cols_optional = ["id"]  # si viene, permite actualizar por ID
    ...
    if up_excel:
        ...
        if df_imp_raw.empty:
            ...
            invalid_suc = ~df_norm["sucursal"].isin(SUCURSALES) & df_norm["sucursal"].ne("")
            invalid_est = ~df_norm["estatus"].isin(ESTATUS_OPCIONES) & df_norm["estatus"].ne("")
            if invalid_suc.any():
                st.error(f"Sucursales inválidas en {invalid_suc.sum()} fila(s). Válidas: {SUCURSALES}.")
            if invalid_est.any():
                st.error(f"Estatus inválidos en {invalid_est.sum()} fila(s). Válidos: {ESTATUS_OPCIONES}.")
